...
**What the layout cannot assert is the host's, and it is four rules**: `assets/*` immutable, no edge cache on `quiver/quiver.json`, a missing path answered as a 404, and https. The first is the client's own bundle, which the loader reads nothing of; the second is the edge alone, the browser layer being closed by the fetch modes the transport picks per name-shape (QUIVER §"The index"). The last two are what a static host's defaults get wrong, and both surface as something other than themselves — a file that fails to parse, and bytes nothing checked. The README carries them with the recipes, where a consumer deploying studio reads, rather than in quiver's.
