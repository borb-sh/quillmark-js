...
| do not cache `quiver/quiver.json` at the edge | the one name in the artifact carrying no digest, and a stale one pins readers to old quills                                                   |
| a missing path is a 404                       | an SPA fallback — the commonest default there is — answers 200 with the client's HTML, which then fails to parse as the file it stands in for |
