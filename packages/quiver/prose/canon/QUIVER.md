...
**Both halves of that are the transport's to say, not a host's.** A name carrying its own digest is entitled to a cached response whatever its age, so the other fetches are `force-cache` rather than left at a default that a host's `max-age=0` turns into a revalidation per name per load. The fact is the artifact's own and no header conveys it better; a host that states it too is agreeing, and one that never states it costs nothing. What a header still owns is the layer above the browser's, and the client's own bundle beside the artifact, which this reads nothing of.
