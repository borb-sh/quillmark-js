<!--
 ...
 (VISUAL_EDITOR §"Structure mirrors the schema", canon `SCHEMAS.md` §Matrix).
 A roster declaring no columns tiles, as many members abreast as the width holds;
 one declaring columns stacks, since a member unfolds inside its own track and a
 track that grows would reflow the members beside it. Each member is a real
 checkbox bound to its `held` cell with
...
>
	...
	<div class="qm-matrix-roster" class:tiled={!hasColumns} bind:this={rosterEl}>
...
<style>
	...
	/* The roster stacks: a member unfolds its columns inside its own box, so a track
	 sized to its content would widen on a tick and reflow the members beside it — the
	 one interaction the control exists for. Positioned for the wash's inset child
	 (`washBox`), and rounded to the rung a row's box draws, as an array's rows are. */
	.qm-matrix-roster {
		position: relative;
		display: flex;
		flex-direction: column;
	...
	/* A roster declaring no columns is a checklist: nothing unfolds, so the members tile
	 instead, as many abreast as the width holds at the track floor, `auto-fit` collapsing
	 the tracks it has no member for. Nothing measures. The row packs at the start: the
	 leftover stands after the last member rather than split between them, where a short
	 vocabulary in a wide field reads as columns set far apart. */
	.qm-matrix-roster.tiled {
		display: grid;
