...
describe('patchEditorExt', () => {
	...
	it('drops a key patched to undefined, keeping the rest — on main and on a card', () => {
		// The dismissal write (`VisualEditor.dismissTips`), and the hazard a
		// namespace-wide write would cause: `tips` and `title` are sibling keys of one
		// namespace, so clearing the channel by replacing the namespace destroys the
