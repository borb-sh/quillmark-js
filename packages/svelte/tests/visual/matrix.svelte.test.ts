...
// Driven off the reference quill's `checks` — six members, two columns — and read
...
describe('a matrix field', () => {
	it('draws the roster as real checkboxes, each titled by a label for it', () => {
		const q = quill();
		mounted = mountEditor(q, q.seedDocument());
		const m = matrix(mounted.target);

		// A roster declaring columns stacks: a tick unfolds inside the member's own box,
		// so nothing beside it is sized off content that a tick reveals.
		expect(m.querySelector('.qm-matrix-roster.tiled')).toBeNull();
	...
	it('walks the roster with the arrow keys', () => {
		...
		press(tick(m, 'Citations'), 'ArrowUp');
		expect(document.activeElement).toBe(spelling);
		// The roster's edge is the walk's: past it the caret stays where it is.
