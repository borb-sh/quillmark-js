...
- **Paint**: one `<canvas>` per visible page (+ margin), painted via `session.paint`; mounted/unmounted on scroll, so memory stays bounded to *visible + margin* per the canon. Each page slot is a `.qm-page-slot` carrying `data-page`, its index: the two hooks a consumer drawing its own overlay targets, the number being otherwise position among siblings, which holds today and is not a contract. A canvas fills its slot (`width`/`height: 100%`), so its ink is always on the box the click and scroll geometry measure, whatever the raster was painted at. Mounted pages repaint on container-width changes (a rAF-coalesced `ResizeObserver`) and on DPR changes (a re-arming `(resolution: …dppx)` query), so a stretched raster is re-sharpened rather than left soft; the canvases are `position:absolute`, so a repaint never feeds back into layout. The raster's density is `devicePixelRatio` floored at 2 (`MIN_DENSITY`), times the host's `setZoom`: glyphs rasterize unhinted, so a 1× or fractional-ratio raster resampled onto its box leaves small text soft, where one at least twice the box is downsampled and reads. When `PaintResult.clamped` caps density on an oversized page, V1 honors it silently.
...
**What either rung can name at depth is the plate's, and a scalar cell is out of reach.** A content value carries its own spans, so a `richtext` or `plaintext` cell regions wherever it is placed — through a loop binding, at any depth: `#for rev in data.revisions [#rev.detail]` regions at `main.revisions[0].detail`. A scalar carries none, so its address is the read path's, and the codegen tracks a literal one only: `data.revisions.at(0).note` regions at `main.revisions[0].note` where `data.revisions.at(i).note` regions at `main.revisions`, the whole array. An array a plate prints by looping — which is every array whose length the document decides — therefore names its rows through their content cells and through nothing else. The reference quill declares one for that reason (`revisions.detail`), so a landing into a nested row is drivable here rather than only in a quill this workspace does not hold.
